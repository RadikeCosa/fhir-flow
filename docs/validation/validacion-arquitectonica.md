...
| G2 — hueco principal de evidencia browser system-wide | **cierre documental acotado (evidencia principal cubierta)** | Se incorporó evidencia browser nueva con corrida verde de `e2e/flows/encounter-cross-surface-no-mix.spec.ts` para el loop `save -> reload/remount -> rehydrate -> patient detail -> finalize -> history -> return detail`. Resultado: **cobertura browser G2 validada en alcance acotado**, **sin bug runtime nuevo verificable** y **sin cambios productivos de app runtime** (fix mínimo de readiness del seed loader). | Spec browser G2 + corrida verde reportada de los escenarios `target encounter stays isolated across detail, patient detail and history` y `save -> reload/remount -> rehydrate -> finalize remains consistent across patient detail and history`. | Mantener límite explícito: **no implica cierre global/system-wide**, **no sustituye G3/G4** y no clausura el frente global por este resultado aislado. |
...
Actualización documental acotada G2 (2026-04-06): quedó cubierto el **hueco principal de evidencia browser de G2** con corrida verde del spec `e2e/flows/encounter-cross-surface-no-mix.spec.ts`; esto deja **cobertura browser G2 validada en alcance acotado**, **sin bug runtime nuevo verificable** y **sin cambios productivos de app runtime**. El resultado **no implica cierre global/system-wide** y **no sustituye G3/G4**.
