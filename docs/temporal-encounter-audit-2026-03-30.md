...
- **Diagnóstico corto (verificado):** el sistema ya persiste vínculo `encounter` al escribir signos vitales y EVA, pero en lectura hay rutas que siguen componiendo por fecha y/o no hidratan `encounterId` en el modelo de dominio, lo que permite mezcla inter-encounter en superficies equivocadas.  
- **Problema raíz principal (verificado):** desalineación entre modelo de escritura encounter-centric y modelo de lectura mixto (encounter + fecha), más una pérdida de `encounterId` en mappers de vitales/EVA para consultas por paciente.  
- **Nivel de riesgo:** **alto** en consistencia clínica de superficies encounter-centric (patient detail/continuidad), **medio** en historial, **bajo** en charts longitudinales (donde la agregación temporal sí es válida si se mantiene explícita).
...
2. Esto facilita que el mapper de vitales no hidrate `encounterId` aunque exista en FHIR real (al ser `.passthrough()` no falla, pero tampoco se aprovecha de forma tipada). **Inferido**.
...
2. Mapper de vitales agrupa por `effectiveDateTime + performer` (`groupKey = date::performer`) y no incluye `encounterId` en el `VitalSignRecord`. **Verificado**.
3. Mapper de EVA tampoco mapea `encounterId`, aunque el recurso lo trae en schema. **Verificado**.
...
2. Aunque `findByEncounterId` filtra en FHIR por `encounter`, los objetos retornados siguen sin `encounterId` por la pérdida en mapper. **Verificado**.
...
1. **Patient detail**: calcula `inProgressEncounter`, pero carga procedimientos/EVA/vitales usando `lastEncounter` (finished por query) y luego expone `lastEncounter = inProgressEncounter ?? lastEncounter`. Esto puede mostrar visita en curso con datos clínicos de otra visita. **Verificado**.
...
1. **Patient detail**: comportamiento actual no es seguro encounter-centric por desacople entre encounter mostrado y payload clínico mostrado. **Verificado**.
...
| `VitalSignRecord.date` | copia de `effectiveDateTime` | patient detail, encounter detail, charts | sin `encounterId` hidratado en mapper | agregar `encounterId` y conservar timestamp completo |
| `EvaAssessment.date` | copia de `effectiveDateTime` | patient detail, history, charts | idem anterior | idem anterior |
...
- **Contrato actual:** `lastEncounter` (sobrescrito con `inProgressEncounter`), y bloques clínicos cargados desde `lastEncounter` original (finished). **Verificado**.
- **Criterio actual:** encounter-centric parcial (queries por encounter) pero con mismatch de encounter mostrado vs datos asociados. **Verificado**.
- **Riesgo:** alto; caso reportado reproducible por lectura desalineada incluso sin fallback por fecha. **Verificado**.
- **Decisión recomendada:** resolver encounter “fuente” único para la sección (preferir `inProgressEncounter ?? lastFinished`) y cargar clínicos para ese mismo ID; sin fallback por fecha en esta surface. **Verificado (recomendación)**.

### Encounter detail
- **Contrato actual:** `encounter`, `vitalSigns`, `evaRecords`, `procedures` por `findByEncounterId` (solo finished). **Verificado**.
- **Criterio actual:** por `encounterId` (correcto para finished). **Verificado**.
...
- **Riesgo:** bajo en persistencia, medio en recuperación por mapper sin `encounterId`. **Verificado**.
- **Decisión recomendada:** corregir hidratación `encounterId` en read mappers para cierre de ciclo. **Verificado (recomendación)**.
...
1. **Bug principal reportado (patient detail):** sección muestra encounter en curso, pero EVA/vitales/procedimientos pueden venir del último encounter finalizado porque el fetch clínico no usa el encounter finalmente mostrado. **Verificado**.
2. **Pérdida de `encounterId` en lectura:** mappers de vitales/EVA no lo hidratan, impidiendo partición robusta posterior y forzando heurísticas por fecha. **Verificado**.
...
2. **Hidratación de linkage (infra):** mapear `encounterId` en vitales/EVA desde `Observation.encounter.reference`; opcionalmente exponer bandera `associationMode`.  
3. **Corrección patient detail (loader):** elegir `encounterSourceId` único y consultar clínicos por ese ID; eliminar mezcla implícita.  
...
1. Patient detail: alinear encounter mostrado y datasets clínicos usando mismo `encounterId`. **Verificado (factible inmediata)**.
2. Mapper EVA/vitales: hidratar `encounterId` en dominio cuando exista referencia. **Verificado (factible inmediata)**.
