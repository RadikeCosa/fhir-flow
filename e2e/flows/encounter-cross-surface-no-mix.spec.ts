...
async function finalizeTargetEncounter(page: Page) {
    ...
    const initialAlertText = ((await page.getByRole("alert").first().textContent().catch(() => "")) ?? "").trim();

    await page.getByRole("button", { name: "Finalizar visita" }).click();
    await Promise.race([
        page
            .waitForURL((url) => !url.pathname.endsWith(`/encounters/${TARGET_ENCOUNTER_ID}`), { timeout: 20000 })
            .then(() => "url-changed")
            .catch(() => null),
        page
            .waitForSelector("text=Esta visita está finalizada y no puede editarse", { timeout: 20000 })
            .then(() => "banner-visible")
            .catch(() => null),
    ]);
...
test("target encounter stays isolated across detail, patient detail and history", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Completar visita" })).toHaveAttribute("href", TARGET_ENCOUNTER_URL);
    ...
    await expect(page.getByRole("heading", { name: "Historial de Encuentros" })).toBeVisible();
    await expect(page.getByText(FINAL_REASON)).toBeVisible();
    await expect(page.getByText(SIBLING_REASON)).toBeVisible();
    await expect(page.locator(`a[href="/patients/${PATIENT_ID}/encounters/${TARGET_ENCOUNTER_ID}"]`)).toBeVisible();
    await expect(page.locator(`a[href="/patients/${PATIENT_ID}/encounters/${SIBLING_ENCOUNTER_ID}"]`)).toBeVisible();
    await expect(page.getByText(PARTIAL_NOTE)).toHaveCount(0);
});